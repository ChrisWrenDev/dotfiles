...
- **Parallel sessions** — `tmux` + git worktrees for concurrent work on multiple tickets
...
| `worktree_create`, `worktree_cleanup`                 | Ask                             |
...
Create a git worktree via the repo's `scripts/create_worktree.sh` script. Accepts an optional branch name, base branch, and `--no-thoughts` flag.

### `worktree_cleanup`

Remove a worktree (`git worktree remove --force`) and optionally delete its branch. Worktrees are expected at `~/wt/<repo>/<name>`.
...
| `/ralph_impl`     | Single-shot: set up a worktree for the highest-priority "ready for dev" ticket                                                                                                 |
...
| Command             | Description                                                               |
| ------------------- | ------------------------------------------------------------------------- |
| `/create_worktree`  | Create a worktree and output tmux launch instructions                     |
| `/cleanup_worktree` | Remove a worktree and optionally delete its branch                        |
| `/launch_session`   | Generate `tmux new-window` commands for a new parallel OpenCode session   |
...
/ralph_impl ENG-1234      →  worktree created, launch instructions printed
```
...
```bash
# 1. Create a worktree
/create_worktree           # or: worktree_create tool directly

# 2. Launch an implementation session
tmux new-window -n "ENG-1234" -c "~/wt/<repo>/ENG-1234" "opencode -m anthropic/claude-opus-4-6"

# 3. In the new session, implement
...
| Context                                         | Model                                    |
| ----------------------------------------------- | ---------------------------------------- |
| Default (agents, worktree setup, most commands) | `sonnet` (`anthropic/claude-sonnet-4-6`) |
| Heavy research and planning commands            | `opus` (`anthropic/claude-opus-4-6`)     |
| Tmux session launch examples                    | `anthropic/claude-opus-4-6`              |
...
- [ ] Confirm `scripts/create_worktree.sh` exists in your target repo (required by `worktree_create`)
